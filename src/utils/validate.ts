...
export const validateUserId = (userId: string): Array<errorMessage> | null => {
	if (!userId) {
	...
	if (userId.length > 20) {
		return [
			{ field: "userId", message: "userId must be 20 characters or less" },
		];
	}
...
export const validateLastName = (
	...
	if (lastName.length > 20) {
		return [
			{ field: "lastName", message: "Last name must be 20 characters or less" },
		];
	}
...
export const validateDob = (dob: Date): Array<errorMessage> | null => {
	if (!dob) {
		return [{ field: "dob", message: "Missing date of birth" }];
	}

	if (dob instanceof Date) {
		return [{ field: "dob", message: "Date of birth must be a date object" }];
	}

	return null;
};

export const validatePhone = (phone: string): Array<errorMessage> | null => {
	if (!phone) {
...
export const validateEmail = (email: string): Array<errorMessage> | null => {
	if (!email) {
